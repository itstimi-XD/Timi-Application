package com.love.timi.controller.v1
...
import com.love.timi.service.v1.UserService
...
@Deprecated(message = "This controller is deprecated, use version 2 instead")
class UserController: BaseController() {
    ...
    fun getIpAddr(): ResponseEntity<RestResponse<UserIpRes>> {
        return RestResponse<UserIpRes>().ok().setBody(userService.getIp()).responseEntity()
    }
    /**
     * 회원가입
     */
    ...
    fun login(@Valid @RequestBody pwVerificationReq: PwVerificationReq, bindingResult: BindingResult): ResponseEntity<RestResponse<PasswordVerificationRes>> {
        val loginResponse = userService.login(pwVerificationReq)
    ...
    fun verifyOtp(@Valid @RequestBody otpVerificationReq: OtpVerificationReq, bindingResult: BindingResult): ResponseEntity<RestResponse<FinLogSuccessRes>> {
        return RestResponse<FinLogSuccessRes>().ok().setBody(userService.verifyOtp(otpVerificationReq)).responseEntity()
    }
    ...
    fun askInfo(): ResponseEntity<RestResponse<UserInfoRes>> {
        return RestResponse<UserInfoRes>().ok().setBody(userService.askUserInfo()).responseEntity()
    }
    ...
    @AuthType([AuthType.Authority.MAIN_MENU_READ])
    fun logout(): ResponseEntity<RestResponse<HashMap<String, Any>>> {
        return RestResponse<HashMap<String, Any>>().ok().setBody(userService.logout()).responseEntity()
    }
    ...
    fun renewToken(): ResponseEntity<RestResponse<AuthenticationRes>> {
        return RestResponse<AuthenticationRes>().ok().setBody(userService.renewToken()).responseEntity()
    }
    // ========================================================================================================================
    // 로그인 > 비밀번호 초기화(재설정)
    // ========================================================================================================================
    ...
    fun changePassword(@Valid @RequestBody changePwReq: ChangePwReq, bindingResult: BindingResult): ResponseEntity<RestResponse<HashMap<String, Any>>> {
        return RestResponse<HashMap<String, Any>>().ok().setBody(userService.changePassword(changePwReq)).responseEntity()
    }
    ...
    fun deferChangePassword(): ResponseEntity<RestResponse<HashMap<String, Any>>> {
        return RestResponse<HashMap<String, Any>>().ok().setBody(userService.deferPwChangeRequiredAt()).responseEntity()
    }
    ...
    fun getMyInfo(): ResponseEntity<RestResponse<MyInfoRes?>> {
        return RestResponse<MyInfoRes?>().ok().setBody(userService.getMyInfo(getUserKey().toLong())).responseEntity()
    }
    /**
     * 회원정보 수정
     */
    ...
    fun updateMyInfo(@Valid @RequestBody updateMyInfoReq: UpdateMyInfoReq, bindingResult: BindingResult): ResponseEntity<out Any> {
        return RestResponse<HashMap<String, Any>>().ok().setBody(userService.modifyMyInfo(updateMyInfoReq)).responseEntity()
    }
